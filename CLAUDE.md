...
### 4. BSR 403エラー根本対応（明日朝以降）

**現状:**
- 親ASIN除外修正（commit `c1e7470`）でB0FKZXVLV4の403は解消済み
- 残る403対象: **B0CF3XL3F9** (RHINON ノートパソコンスタンド ms02), **B0GMFS1PTY** (imin ホワイトセージ お得用 100g sage_100)
- 両方とも DB上は `is_parent=false`, `parent_asin=""` の単独商品
- なぜ403が返るか不明（→明日のログで判別）
- **業務影響:** BSR以外のチャネル（Orders/Inventory/Ads/楽天/Shopify/Meta）は正常動作。緊急性低

**明日朝（JST 02:00 の自動Cron実行後）にやること:**

1. **Vercel Logs で詳細エラーメッセージを確認**
   - 検索: `requestPath:/api/cron/sync`
   - `[BSR] B0CF3XL3F9 / B0GMFS1PTY の Error details` ログを見る
   - 段階1で拡張したログから `spApiBody.errors[].code` と `errors[].message` を取得

2. **SP-APIが返している実際のエラーコードを確認:**
   - `Unauthorized` → Brand Registry / Listing所有権問題
   - `Forbidden` → 販売不可ASIN / Marketplace設定
   - その他のコード → 別途調査

3. **原因に応じて対処方針を決定:**
   - Brand Registry問題 → Seller Centralで申請（コード変更不要）
   - Listing所有権 → 403/404をcatchして `is_archived=true` にする自己修復ロジック実装
   - ロール不足 → Seller Centralで権限再付与
   - 親ASINだった場合 → DB手動更新で `is_parent=true` に（`UPDATE products SET is_parent=true WHERE asin IN (...)`）

4. **必要なら段階2（デバッグエンドポイント追加）に進む:**
   - `src/app/api/debug/bsr-test/route.ts`（既存の getCatalogItemBSR を呼び出して結果取得）
   - `src/app/api/debug/bsr-raw/route.ts`（生の fetch でレスポンスヘッダーまで取得）
   - **一時的なデバッグ用、対処完了後に削除すること**

5. **確認対象ASIN（明日の自動Cronで403が出るはず）:**
   - B0CF3XL3F9 (RHINON ノートパソコンスタンド ms02)
   - B0GMFS1PTY (imin ホワイトセージ お得用 100g sage_100)
   - 他にも出るかもしれない（要全件確認）
